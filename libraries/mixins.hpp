...
#pragma message("Heckerpowered.Mixins v1.0.1.22623 build.")
...
namespace mixins
{
	...
	__mixin_forceinline [[nodiscard]] inline std::basic_string<char_type> hex(T value) noexcept
	{
		...
		{
			char buffer[2]{ _Hex(value), '\0' };
			return std::basic_string<char_type>(buffer, 2);
		}
	}
	...
	/// <returns>Checksum.</returns>
	template<typename T>
	__mixin_forceinline constexpr unsigned const compute_crc32(const T& value) noexcept
	{
	...
	inline bool request_init_mixin() noexcept
	{
		...
		if (!init_service())
		{
			...
			else
			{
				...
				TASKDIALOG_BUTTON button[]{ {.nButtonID = 0,.pszButtonText = __mixin_xor_string(L"使用其他凭据重新启动\n重新启动Mixin, 系统会提示您更改用户账户。") },
					{.nButtonID = 1,.pszButtonText = __mixin_xor_string(L"取消任务并返回到Mixin") } };
				...
				config.cButtons = 2;
				config.pszMainInstruction = __mixin_xor_string(L"此任务要求应用程序具有提升的权限。");
				...
				config.pszExpandedControlText = __mixin_xor_string(L"查看错误消息");
				config.pszExpandedInformation = __mixin_xor_string(L"无法初始化Mixin Service。Mixin Service没有足够的特权初始化Mixin Service。要初始化Mixin Service, 必须以管理员身份运行Mixin。");
	...
	inline std::wstring convert(std::string const& value) noexcept
	{
		std::wstring buffer;
		buffer.resize(value.size());
		MultiByteToWideChar(CP_ACP, 0, value.data(), -1, buffer.data(), static_cast<int>(buffer.size()));
		return buffer;
	}

	inline std::string convert(std::wstring const& value) noexcept
	{
		std::string buffer;
		buffer.resize(value.size());
		WideCharToMultiByte(CP_ACP, 0, value.data(), -1, buffer.data(), static_cast<int>(buffer.size()),
			nullptr, nullptr);
		return buffer;
	}
}
